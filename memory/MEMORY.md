...
- [★★내 블로그 imgsrc_ 노출붕괴](project_imgsrc_blog_collapse.md) — ★260823 **전제정정: 내 블로그=imgsrc_(방구석 리뷰어), 홍기자·우리끼리는 남의 블로그**(targets.json self 오표기). ★**홈판 64%→10%**(7/20~8/03·검색은 원래 3~8%) · **신규글 442배 붕괴**(55,121→292)·기존글 50% 생존 · ★★**단독 사건 확정**(동종 4곳은 TOP10 전부 8월글, 나만 9/10이 7월글) · 성공패턴=**회고형 낙차**(뉴스형의 2.4배·양 아니라 유형) · ★API=cv-ranks는 **서명 때문에 재요청 불가 → fetch 후킹만** · 폐기가설 9건
