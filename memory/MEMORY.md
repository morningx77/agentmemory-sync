...
- [워크스페이스 회사/개인 분리(계획확정·실행대기)](project_workspace_company_personal_split.md) — ★260804 하위폴더안(ISPARK\·IMGINE\)+루트CLAUDE.md범용화. 회사=FIGMA/ISPARK_MCP/CNOTE+02_office/04_design/07_archive, 개인=forest/MATO/Chrome/THEMONY/TSMIN/DEEPNAUT/skills, 공용=000000_INFO/05_notes(A). 검증=CLAUDE.md만 상위상속(누수)·settings/MCP/메모리 격리. 다른세션 완료후 실행
