...
- [컴팩트 권유는 70% 가까이서만](feedback_compact_threshold_relaxed.md) — 260914 사장님 「50%면 여유많아」. 50~60%대는 킥오프만 갱신하고 계속
...
- [★agentmemory 사고 2건 + 260916 규약에서 제외](project_agentmemory.md) · [회사↔집 동기화](project_agentmemory_sync_cross_pc.md) — ★LISTEN≠정상(/health로 판정)·cwd상속하면 빈 저장소 생성. **보조기억일 뿐 1차사료=저널·대장** · ★260916 세션시작 절차에서 빠짐(저널→핸드오프→파일메모리→트랜스크립트) · lessons 전용으로 남김
...
- [★FP 보고 규율](feedback_fp_report_data_only.md) — 실측→뜻→과거 사례 · 훈계·대리결론 금지 · ★260919 "아니다"만 말고 층별 설명(강함/약함/모름) · 시나리오 요청 시 길게(✓/→ 표시·조기신호·무효화조건) · 자청 프롬프트 정본 경로
