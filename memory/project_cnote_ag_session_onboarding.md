...
metadata: 
  ...
  modified: 2026-09-14T13:48:04.415Z
---
...
- **게이트 = 완료의 정의.** ★**게이트 수를 외우지 마라 — 세는 법을 외워라**: `storybook/package.json` 의 `gate` 체인을 센다(`gate:render` 는 별도 · 브라우저 필요).
  ★근거 = 이 수는 **계속 늘어난다.** 온보딩 §5 「10/10」 → 260903 실측 **12** → 260909 실측 **13**(`check-sheet-embedded` 추가). 문서마다 10·12·13·14 가 섞여 있었다.
  ★★즉 **수를 적어 두면 반드시 낡는다** — 260909 에 이 저장소가 같은 문장 하나를 **7자리**에서 발견했고 반나절에 두 번 낡았다. 원인은 값이 아니라 **복사**다. → [[feedback_scope_before_amplifying_alarm]]
...
- ★**handoff_to 에 내 별칭이 든 decision · assign 은 제목 앞부분만 보고 넘기지 말고 전문(▶ 내 별칭 줄)을 읽는다.**
  **Why:** 260914 ag스토리북2 가 5171 을 「FU-2609149 원인」 건으로만 보고 넘겼는데, 같은 줄 뒤에 「B17 device 를 둘로 나눔(새 부품 = ag스토리북)」 이 있었다 → 내 claim · 서브에이전트가 같은 부품을 잡아 겹침(파일 작성 전 5178 로 정리).
  **How to apply:** 착수 claim 을 쓰기 직전에 최근 30분 저널에서 「▶ 내 별칭」 줄을 grep 해 범위를 대조한다 · 모니터 요약(잘린 한 줄)은 판단 근거가 아니다.
